...
- (void)testPush
{
    ...
    NSArray* dirtyDocs = [people allDirtyAndReturnError:nil];
    
    XCTAssertTrue(dirtyDocs.count == 2, @"check dirty document count");
    
    //TIP: Handle the dirty documents here (e.g. send them to an adapter)
    
    //Mark dirty documents as clean
    int numCleaned = [[people markDocumentsClean:dirtyDocs error:nil] intValue];
    
    XCTAssertTrue(numCleaned == 2, @"check number of documents cleaned");
}
