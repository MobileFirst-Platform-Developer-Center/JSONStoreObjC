...
@interface CountAllDirty : XCTestCase

@end

@implementation CountAllDirty

- (void)testCountAllDirty
{
    ...
    [[JSONStore sharedInstance] openCollections:@[people] withOptions:nil error:&error];
    
    //Add data.
    [[people addData:@[@{@"name" : @"carlos", @"age" : @20}, @{@"name" : @"mike", @"age" : @30}] andMarkDirty:YES withOptions:nil error:nil] intValue];
    
    //Count dirty documents.
    int countAllDirtyResult = [[people countAllDirtyDocumentsWithError:nil] intValue];
    
    XCTAssertTrue(countAllDirtyResult == 2, @"check count all result");
}
